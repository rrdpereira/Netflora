...
def get_latest_exp_directory(base_path):
    ...
    exp_directories = [d for d in base_directory.iterdir() if d.is_dir() and re.match(r"^exp(\d+)?$", d.name)]
    if exp_directories:
        exp_directories.sort(key=lambda x: int(re.findall(r'\d+', x.name)[0]) if re.findall(r'\d+', x.name) else 0)
        return exp_directories[-1]  # Return the most recent directory
    else:
        return None
...
def calculate_score(bb, alpha):
    ...
    return (1 - alpha) * area_score + alpha * shape_score
    ...
    for i in range(len(gdf)):
        for j in range(i + 1, len(gdf)):
            ...
            if iou > iou_threshold:
                ...
                to_remove.append(j if score_bb1 > score_bb2 else i)
    return gdf.drop(to_remove)

def filter_rectangular_bounding_boxes(gdf, min_aspect_ratio, max_aspect_ratio):
    rectangular_indices = [i for i, row in gdf.iterrows() if min_aspect_ratio <= (row['geometry'].bounds[3] - row['geometry'].bounds[1]) / (row['geometry'].bounds[2] - row['geometry'].bounds[0]) <= max_aspect_ratio]
    return gdf.loc[rectangular_indices].copy()
...
def plot_class_distribution(gdf_nms_final, output_dir, algorithm):
    ...
    for bar, color in zip(bars, colors):
        ...
        ax.annotate(f'{int(height)}', xy=(bar.get_x() + bar.get_width() / 2, height), xytext=(0, 3), textcoords="offset points", ha='center', va='bottom', color='black')
...
def main():
    ...
    parser.add_argument('--iou', type=float, default=0.20, help="IOU threshold for Non-Maximum Suppression.")
    ...
    output_shapefile_directory = os.path.join(output_dir, "shapefiles/")
    output_csv_directory = os.path.join(output_dir, "csv/")
    ...
    if not latest_exp_directory:
        print("Nenhuma detecção encontrada. Execute primeiro o processo de detecção.")
        return
    ...
    for arquivo_txt in arquivos_txt:
        ...
        utm_width, utm_height = utm_xmax - utm_xmin, utm_ymax - utm_ymin
        with open(arquivo_txt, "r") as txt_file:
            for line in txt_file:
                ...
                bb_xmin, bb_ymin = bb_xcenter - (width * utm_width / 2), bb_ycenter - (height * utm_height / 2)
                bb_xmax, bb_ymax = bb_xmin + width * utm_width, bb_ymin + height * utm_height
    ...
    min_aspect_ratio, max_aspect_ratio = 0.5, 2
    gdf_filtered = filter_rectangular_bounding_boxes(gdf, min_aspect_ratio, max_aspect_ratio)
    gdf_nms = apply_nms_with_score(gdf_filtered.reset_index(drop=True), args.iou, alpha=0.20)

    # Species mapping
    species_category = categories.get(algorithm, [])
    species_codes = [item["specie"] for item in species_category]
    gdf_nms['name'] = gdf_nms['class_id'].apply(lambda x: species_dict.get(species_codes[int(x)], {'common_name': 'Desconhecido'})['common_name'] if int(x) < len(species_codes) else 'Desconhecido')
    gdf_nms['sci_name'] = gdf_nms['class_id'].apply(lambda x: species_dict.get(species_codes[int(x)], {'scientific_name': 'Desconhecido'})['scientific_name'] if int(x) < len(species_codes) else 'Desconhecido')
